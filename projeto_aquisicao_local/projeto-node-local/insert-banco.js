...
const registros_mantidos_tabela_leitura = 100;
...
function registrar_leitura(temperatura, umidade) {
    ...
    banco.conectar().then(() => {
        ...
        INSERT into leitura (fkTrafo, temperatura, umidade, tempoDataLeitura)
        values (1, ${temperatura}, ${umidade}, CONVERT(Datetime, '${agora()}', 120));
        
        delete from leitura where idLeitura not in 
        (select top ${registros_mantidos_tabela_leitura} idLeitura from leitura order by idLeitura desc);`);
...
if (gerar_dados_aleatorios) {
	...
	setInterval(function() {
		...
		registrar_leitura(parseInt(Math.random()*150), parseInt(Math.random()*100))
